...
export const UserProfile: React.FC<Props> = ({ authData, setAuthData }) => {
  ...
  useEffect(() => {
    ...
    const getTotalLikes = async () => {
      const resp = await fetch(
        `https://brave-blog-api.herokuapp.com/api/v1/posts/likes/${authData.username}`,
        {
          headers: {
  ...
  const updateProfile = async (
    ...
    if (
      newUsername !== authData.username ||
      newEmail !== authData.email ||
      newImage !== authData.picUrl
    ) {
      const resp = await fetch(
        "https://brave-blog-api.herokuapp.com/updateUser",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <a.div style={props} className="text-center  flex flex-col items-center">
      <div className="flex items-center px-1 py-2 bg-yellow-200 rounded-sm w-full mb-2 justify-center">
        ...
        <span className="ml-1 font-bold md:text-base text-sm">
          If you change your username you need to re-login with your new
          credentials
        </span>
      </div>
      <a href={authData.picUrl} target="_blank" rel="noreferrer">
        <img
          src={authData.picUrl}
          alt="userProfilePic"
          className="rounded-full ring-2 ring-gray-700 w-60 h-60"
        />
      </a>
      ...
      <form onSubmit={handleSubmit}>
        <div className="mt-4">
          ...
          <input
            ...
            placeholder="Email"
            className="placeholder-gray-700 ring-1 ring-black focus:ring-2 focus:outline-none px-1 py-1 rounded-sm sm:ml-2 sm:mt-0 mt-4"
            required
          />
        </div>
        <input
          ...
          className="placeholder-gray-700 ring-1 ring-black focus:ring-2 focus:outline-none px-1 py-1 rounded-sm mt-4 w-full"
          required
        />
        <button
          type="submit"
          className=" bg-black  hover:bg-gray-800 cursor-pointer text-white px-2 py-1 mt-4 w-full rounded-sm"
        >
          Update infromation
        </button>
      </form>
