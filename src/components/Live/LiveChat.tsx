...
export const LiveChat:React.FC<Props> = ({allComments}) => {
    ...
    return (
        ...
        return(<div key={`${comment.user}++${comment.comment}`}  className="ml-2 break-all"><span className={`font-bold text-${colors[Math.floor(Math.random()*colors.length)]}-600`}>{comment.user} :</span> {comment.comment}</div>)
    })}
